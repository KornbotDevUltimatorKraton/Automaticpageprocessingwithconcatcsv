...
def Mergecsvalgorithm(Pathdata):
       ...
       for ir in range(0,len(listreadymerge)): 
                   ...
                   print(all_filesort)
                   [pd.read_csv(f).reset_index(inplace=True, drop=True) for f in all_filesort]
                   combined_csv = pd.concat([pd.read_csv(f) for f in all_filesort],axis=0) # Concatinate the file 
